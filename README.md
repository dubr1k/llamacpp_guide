...
| `--context-shift`   | Заблокировать модель в RAM, предотвращая swapping* |
...
Флаг `--context-shift` в llama.cpp блокирует модель в RAM, запрещая системе выгружать её в swap. Это гарантирует стабильную производительность, но требует достаточного объёма физической памяти для всей модели.
...
**Когда использовать `--context-shift`:**
...
**Комбинация `--no-mmap --context-shift`:**
...
### 2. Context Shift (`--context-shift`) — «бесконечный» диалог без потери инструкций

По умолчанию в `llama-server`, когда суммарный объем переписки превышает размер контекстного окна (`-c`), сервер либо возвращает ошибку переполнения контекста, либо сбрасывает сессию целиком, заставляя клиент пересчитывать префилл заново.

Флаг `--context-shift` активирует **интеллектуальное скользящее окно (rolling context window)**:
- **Сохранение системного промпта:** Самый первый блок токенов (системный промпт, правила ассистента, инъекции инструментов) закрепляется в начале кэша и **никогда не стирается**.
- **Ротация истории:** Когда контекст заполняется, сервер сдвигает окно, удаляя самые старые реплики пользователя и ассистента из середины истории, освобождая место для новых токенов.
- **Мгновенный отклик без ре-префилла:** Поскольку префикс системы сохранен в KV-кэше, отклик на новые сообщения генерируется мгновенно, без повторного прогона всего контекста.

```bash
llama-server -m model.gguf -c 32768 --context-shift --port 8080 -ngl 999
```

### 3. Как работает кэширование в `llama-server` (RAM vs Диск)

Частый вопрос: *«Почему в llama-server не используется флаг `--prompt-cache`?»*

- **В оперативной памяти (по умолчанию):** Серверная архитектура слотов автоматически сравнивает входящий запрос с предыдущими в рамках сессии. Если системный промпт или начало диалога идентичны, сервер **автоматически переиспользует готовый KV-кэш из RAM**. В логах это отображается как `slot ... KV cache reused ... tokens`. Повторный префилл не выполняется.
- **Сохранение между перезапусками сервера (`--slot-save-path`):** При перезапуске процесса сервера память очищается. Чтобы с дампом кэша ничего не происходило и он восстанавливался с SSD, укажите директорию сохранения:
  ```bash
  --slot-save-path ~/.cache/llama-server-slots
  ```
...
```bash
llama-cli -m models/model.gguf -ngl 99 -c 8192 -b 512 -t 8 -tb 12 -n 2048 --temp 0.7 --repeat-penalty 1.1 --flash-attn on --context-shift --color -i -ins
```
...
```bash
llama-cli -m models/model.gguf -ngl 99 -c 8192 -b 128 -ub 128 -t 4 -tb 12 -n 4096 --flash-attn on --context-shift --no-mmap --color -i -ins
```
...
```bash
llama-server -m models/model.gguf --host 0.0.0.0 --port 8080 -ngl 99 -c 8192 -t 8 -tb 12 -n 4096 --flash-attn on --context-shift --cont-batching --n-parallel 4 --metrics
```

**Сервер для Apple Silicon (минимальный нагрев):**
```bash
llama-server -m models/model.gguf --port 8080 -ngl 99 -c 8192 -b 128 -ub 128 -t 4 -tb 12 -n 4096 --flash-attn on --context-shift --no-mmap
```
...
```bash
llama-server   -m models/model-Q4_K_M.gguf   --port 8080   -ngl 999   --flash-attn on   -b 2048   -ub 1024   -t 8   -tb 8   -ctk q8_0   -ctv q8_0   -c 32768   --context-shift
```
*Разбор конфигурации:*
...
- `--context-shift`: блокирует модель в RAM, предотвращая задержки от swap.

### Apple Silicon: Режим тишины (ноутбук на коленях)

```bash
llama-server -m models/model.gguf --port 8080 -ngl 999 -c 16384 -b 256 -ub 128 -t 4 -tb 6 -ctk q8_0 -ctv q8_0 --flash-attn on --context-shift
```
