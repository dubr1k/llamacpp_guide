...
| `--ctx-shift`   | Заблокировать модель в RAM, предотвращая swapping* |
...
Флаг `--ctx-shift` в llama.cpp блокирует модель в RAM, запрещая системе выгружать её в swap. Это гарантирует стабильную производительность, но требует достаточного объёма физической памяти для всей модели.
...
### 2. Context Shift (`--ctx-shift`) — «бесконечный» диалог без потери инструкций
...
Флаг `--ctx-shift` активирует **интеллектуальное скользящее окно (rolling context window)**:
...
```bash
llama-server -m model.gguf -c 32768 --ctx-shift --port 8080 -ngl 999
```
...
```bash
llama-cli -m models/model.gguf -ngl 99 -c 8192 -b 512 -t 8 -tb 12 -n 2048 --temp 0.7 --repeat-penalty 1.1 --flash-attn on --ctx-shift --color -i -ins
```
...
```bash
llama-cli -m models/model.gguf -ngl 99 -c 8192 -b 128 -ub 128 -t 4 -tb 12 -n 4096 --flash-attn on --ctx-shift --no-mmap --color -i -ins
```
...
```bash
llama-server -m models/model.gguf --host 0.0.0.0 --port 8080 -ngl 99 -c 8192 -t 8 -tb 12 -n 4096 --flash-attn on --ctx-shift --cont-batching --n-parallel 4 --metrics
```

**Сервер для Apple Silicon (минимальный нагрев):**
```bash
llama-server -m models/model.gguf --port 8080 -ngl 99 -c 8192 -b 128 -ub 128 -t 4 -tb 12 -n 4096 --flash-attn on --ctx-shift --no-mmap
```
...
```bash
llama-server   -m models/model-Q4_K_M.gguf   --port 8080   -ngl 999   --flash-attn on   -b 2048   -ub 1024   -t 8   -tb 8   -ctk q8_0   -ctv q8_0   -c 32768   --ctx-shift
```
*Разбор конфигурации:*
...
- `--ctx-shift`: блокирует модель в RAM, предотвращая задержки от swap.

### Apple Silicon: Режим тишины (ноутбук на коленях)

```bash
llama-server -m models/model.gguf --port 8080 -ngl 999 -c 16384 -b 256 -ub 128 -t 4 -tb 6 -ctk q8_0 -ctv q8_0 --flash-attn on --ctx-shift
```
